...
class Walk(object):
    def __init__(self):
        ...
        m.reference_x = Param(  # 参考x(t)
            RangeSet(0, N-1), initialize={0: 0., 1: 0., 2: 0.}, mutable=True)
        ...
        m.referenceobj = m.wg[0]*sum((m.s[2, k]-m.reference_x[k]) ** 2*1
                                     ...
                                     +(m.s[1, k]-m.reference_fy[k])**2*1 for k in m.sk)
    ...
    def Solve(self, state, reference):
        ...
        self.iN.reference_theta.reconstruct(
            {0: reference[0, 0], 1: reference[0, 1], 2: reference[0, 2]})
        self.iN.reference_x.reconstruct(
            {0: reference[1, 0], 1: reference[1, 1], 2: reference[1, 2]})
        self.iN.reference_fy.reconstruct(
            {0: reference[2, 0], 1: reference[2, 1], 2: reference[2, 2]})
