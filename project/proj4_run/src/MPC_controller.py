...
class Walk:
    def __init__(self, state, reference, predict_step=6):
        # state be a 1 * 6 array
        # reference be a 3 * predict_step array, which concludes theta fy and x reference
        m = ConcreteModel()

        m.predict_step = RangeSet(1, predict_step)
        m.minus_1_predict_step = RangeSet(1, predict_step - 1)
        m.minus_2_predict_step = RangeSet(1, predict_step - 2)
        ...
        m.wg = Param(RangeSet(0, 4), initialize={0: 5000000, 1: 5000000, 2: 5000000, 3: 1, 4: 1})
        ...
        m.reference_theta = Param(m.predict_step,
                                  initialize={1: reference[0][0], 2: reference[0][1], 3: reference[0][2],
                                              4: reference[0][3], 5: reference[0][4], 6: reference[0][5]})
        m.reference_fy = Param(m.predict_step,
                               initialize={1: reference[1][0], 2: reference[1][1], 3: reference[1][2],
                                           4: reference[1][3], 5: reference[1][4], 6: reference[1][5]})
        m.reference_x = Param(m.predict_step,
                              initialize={1: reference[2][0], 2: reference[2][1], 3: reference[2][2],
                                          4: reference[2][3], 5: reference[2][4], 6: reference[2][5]})
