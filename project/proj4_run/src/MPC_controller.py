...
class Fly:
    def __init__(self, state, reference):
        ...
        m.minus_1_predict_step = RangeSet(1, length4reference - 1)
        m.minus_2_predict_step = RangeSet(1, length4reference - 2)
        m.State = RangeSet(1, 10)

        # Parameters
        # weight of item in objective function
        # {0: theta position error, 1: fy position error,
        #  2: moment value, 3: rate of moment's change}
        m.wg = Param(RangeSet(0, 3), initialize={0: 10, 1: 10, 2: 0.00001, 3: 0})
        ...
        m.reference_y = Param(m.predict_step, initialize=ref4y)

        # Variables
        # m.s: {1: theta, 2: fy, 3: x, 4: v_theta, 5: v_fy, 6: v_x, 7: y, 8: v_y, 9: alpha, 10: v_alpha}
        m.s = Var(m.State, m.predict_step)
        m.M1 = Var(m.predict_step, bounds=(-250, 250))
        m.M2 = Var(m.predict_step, bounds=(-250, 250))
        ...
        def rule_v_x(model, i):
            return 0.0 == 1.0*m.M*((m.s[6, i + 1] - m.s[6, i]) / m.dt) + \
                   m.mb*(-m.l1*sin(m.s[1, i])*m.s[4, i]**2 + m.l1*cos(m.s[1, i])*((m.s[4, i + 1] - m.s[4, i]) / m.dt) -
                         m.l2**m.s[5, i]*(m.s[5, i] + m.s[4, i])*sin(m.s[2, i] + m.s[1, i]) +
                         m.l2**m.s[5, i]*math.log(m.l2)*cos(m.s[2, i] + m.s[1, i]) *
                         ((m.s[5, i + 1] - m.s[5, i]) / m.dt) + ((m.s[6, i + 1] - m.s[6, i]) / m.dt))
        m.v_x_update = Constraint(m.minus_1_predict_step, rule=rule_v_x)

        def rule_v_y(model, i):
            return 0.0 == m.M*m.g + 1.0*m.M*((m.s[8, i + 1] - m.s[8, i]) / m.dt) + m.g*m.mb - \
                   1.0*m.mb*(m.l1*sin(m.s[1, i])*((m.s[4, i + 1] - m.s[4, i]) / m.dt) +
                             m.l1*cos(m.s[1, i])*m.s[4, i]**2 + m.l2*(m.s[5, i] + m.s[4, i]) *
                             cos(m.s[2, i] + m.s[1, i])*m.s[5, i] + m.l2*sin(m.s[2, i] + m.s[1, i]) *
                             ((m.s[5, i + 1] - m.s[5, i]) / m.dt) - ((m.s[8, i + 1] - m.s[8, i]) / m.dt))
        m.v_y_update = Constraint(m.minus_1_predict_step, rule=rule_v_y)

        def rule_v_theta(model, i):
            return m.M1[i] + m.M2[i] == m.Ib*((m.s[5, i + 1] - m.s[5, i]) / m.dt) + \
                   m.Ib*((m.s[4, i + 1] - m.s[4, i]) / m.dt) - m.g*m.l1*m.mb*sin(m.s[1, i]) - \
                   m.g*m.l2*m.mb*sin(m.s[2, i] + m.s[1, i]) + m.l1**2*m.mb*((m.s[4, i + 1] - m.s[4, i]) / m.dt) + \
                   m.l1*m.l2*m.mb*sin(m.s[2, i] + 2*m.s[1, i])*m.s[5, i]**2/2 - \
                   m.l1*m.l2*m.mb*sin(m.s[2, i])*m.s[5, i]**2/2 - \
                   m.l1*m.l2*m.mb*cos(m.s[2, i] + 2*m.s[1, i])*((m.s[5, i + 1] - m.s[5, i]) / m.dt)/2 + \
                   m.l1*m.l2*m.mb*cos(m.s[2, i])*((m.s[5, i + 1] - m.s[5, i]) / m.dt)/2 + \
                   m.l1*m.l2**m.s[5, i]*m.mb*math.log(m.l2)*cos(m.s[2, i] + 2*m.s[1, i]) *\
                   ((m.s[5, i + 1] - m.s[5, i]) / m.dt)/2 + m.l1*m.l2**m.s[5, i]*m.mb*math.log(m.l2)*cos(m.s[2, i]) *\
                   ((m.s[5, i + 1] - m.s[5, i]) / m.dt)/2 - m.l1*m.l2**m.s[5, i]*m.mb *\
                   sin(m.s[2, i] + 2*m.s[1, i])*m.s[5, i]/2 - m.l1*m.l2**m.s[5, i]*m.mb*sin(m.s[2, i])*m.s[5, i]/2 - \
                   m.l1*m.mb*sin(m.s[1, i])*((m.s[8, i + 1] - m.s[8, i]) / m.dt) + \
                   m.l1*m.mb*cos(m.s[1, i])*((m.s[6, i + 1] - m.s[6, i]) / m.dt) - \
                   m.l2**2*m.mb*sin(2*m.s[2, i] + 2*m.s[1, i])*m.s[5, i]**2/2 + \
                   m.l2*m.mb*cos(m.s[2, i] + m.s[1, i])*m.s[5, i]*m.s[8, i] + \
                   m.l2**(2*m.s[5, i])*m.mb*sin(2*m.s[2, i] + 2*m.s[1, i])/2 + \
                   m.l2**m.s[5, i]*m.mb*sin(m.s[2, i] + m.s[1, i])*m.s[6, i]
        m.v_theta_update = Constraint(m.minus_1_predict_step, rule=rule_v_theta)

        def rule_v_fy(model, i):
            return -m.M1[i] == m.Ib*(((m.s[5, i + 1] - m.s[5, i]) / m.dt) + ((m.s[4, i + 1] - m.s[4, i]) / m.dt)) - \
                   m.g*m.l2*m.mb*sin(m.s[2, i] + m.s[1, i]) - \
                   1.0*m.mb*(m.l2*(m.l1*sin(m.s[1, i])*m.s[4, i] + m.l2*sin(m.s[2, i] + m.s[1, i])*m.s[5, i] -
                                   m.s[8, i])*cos(m.s[2, i] + m.s[1, i])*m.s[5, i] -
                             m.l2**m.s[5, i]*(m.l1*cos(m.s[1, i])*m.s[4, i] + m.l2**m.s[5, i]*
                                              cos(m.s[2, i] + m.s[1, i]) + m.s[6, i])*sin(m.s[2, i] + m.s[1, i])) + \
                   m.mb*(m.l2*(m.s[5, i] + m.s[4, i])*(m.l1*sin(m.s[1, i])*m.s[4, i] +
                                                       m.l2*sin(m.s[2, i] + m.s[1, i])*m.s[5, i] -
                                                       m.s[8, i])*cos(m.s[2, i] + m.s[1, i]) +
                         m.l2*(m.l1*sin(m.s[1, i])*((m.s[4, i + 1] - m.s[4, i]) / m.dt) +
                               m.l1*cos(m.s[1, i])*m.s[4, i]**2 + m.l2*(m.s[5, i] + m.s[4, i])*
                               cos(m.s[2, i] + m.s[1, i])*m.s[5, i] + m.l2*sin(m.s[2, i] + m.s[1, i]) *
                               ((m.s[5, i + 1] - m.s[5, i]) / m.dt) -
                               ((m.s[8, i + 1] - m.s[8, i]) / m.dt))*sin(m.s[2, i] + m.s[1, i]) -
                         m.l2**m.s[5, i]*(m.s[5, i] + m.s[4, i])*(m.l1*cos(m.s[1, i])*m.s[4, i] +
                                                                  m.l2**m.s[5, i]*cos(m.s[2, i] + m.s[1, i]) +
                                                                  m.s[6, i])*math.log(m.l2)*sin(m.s[2, i] + m.s[1, i]) +
                         m.l2**m.s[5, i]*(m.l1*cos(m.s[1, i])*m.s[4, i] + m.l2**m.s[5, i] *
                                          cos(m.s[2, i] + m.s[1, i]) + m.s[6, i])*math.log(m.l2)**2 *
                         cos(m.s[2, i] + m.s[1, i])*((m.s[5, i + 1] - m.s[5, i]) / m.dt) + m.l2**m.s[5, i] *
                         (-m.l1*sin(m.s[1, i])*m.s[4, i]**2 + m.l1*cos(m.s[1, i])*((m.s[4, i + 1] - m.s[4, i]) / m.dt) -
                          m.l2**m.s[5, i]*(m.s[5, i] + m.s[4, i])*sin(m.s[2, i] + m.s[1, i]) +
                          m.l2**m.s[5, i]*math.log(m.l2)*cos(m.s[2, i] + m.s[1, i]) *
                          ((m.s[5, i + 1] - m.s[5, i]) / m.dt) + ((m.s[6, i + 1] - m.s[6, i]) / m.dt)) *
                         math.log(m.l2)*cos(m.s[2, i] + m.s[1, i]))
        m.v_fy_update = Constraint(m.minus_1_predict_step, rule=rule_v_fy)

        def rule_v_alpha(model, i):
            return -m.M2[i] == 1.0*m.Ir*((m.s[10, i + 1] - m.s[10, i]) / m.dt)
        m.v_alpha_update = Constraint(m.minus_1_predict_step, rule=rule_v_alpha)

        # Objective function
        m.referenceObj = sum((m.s[1, k] - m.reference_theta[k]) ** 2 * m.wg[0] +
                             (m.s[2, k] - m.reference_fy[k]) ** 2 * m.wg[1] for k in m.minus_1_predict_step)

        m.M1obj = m.wg[2] * sum(m.M1[k] ** 2 for k in m.minus_1_predict_step)
        m.M2obj = m.wg[2] * sum(m.M2[k] ** 2 for k in m.minus_1_predict_step)

        m.sM1obj = m.wg[3] * sum((m.M1[k + 1] - m.M1[k]) ** 2 for k in m.minus_2_predict_step)
        m.sM2obj = m.wg[3] * sum((m.M2[k + 1] - m.M2[k]) ** 2 for k in m.minus_2_predict_step)
